      ...
      if (retrive == false) {
        $.ajax(

          {
            ...
            contentType: "application/json; charset=utf-8",
            data: send,
            url: 'http://localhost:1222/createNewUser',
            success: function (data) {
...
<script>
  ...
  $(".defOfWord>.textSetting").focusout(function () {
    var cur = $(this).val().trim();
    var index_click = $(this).parent().parent().parent().find(".flashStep").text();
    var card = $(".card_word_wrapper");
    if (cur != "") {
      card.each(function () {
        var index_chek = $(this).find(".flashStep").text();

        if ((index_click != index_chek) && cur == $(this).find(".defOfWord>.textSetting").val().trim()) {

          $(this).find(".defOfWord>.textSetting").val($(this).find(".defOfWord>.textSetting").val() + "(2)")
        }
      })
    }
  }).blur()
